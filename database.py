...
def get_db():
    ...
    if 'db' not in g:
        db_path = current_app.config.get('DB_PATH', 'data/caravax.db')
...
def get_blutentnahmen_einrichtung(einrichtung_id, stichtag=None):
    ...
    sql = (
        ...
        "AND (b.naechste_faelligkeit IS NOT NULL OR b.status = 'OFFEN') "
        "AND ("
        "    (b.status = 'OFFEN' AND b.naechste_faelligkeit < ?)"
        "    OR "
        "    (b.status = 'OFFEN' AND b.naechste_faelligkeit >= ? AND b.naechste_faelligkeit <= ?)"
        "    OR "
        "    (b.status = 'DURCHGEFUEHRT' AND b.naechste_faelligkeit >= ? AND b.naechste_faelligkeit <= ?)"
        ")"
        "ORDER BY "
        "  CASE WHEN b.status = 'OFFEN' AND b.naechste_faelligkeit < ? THEN 0 ELSE 1 END, "
        "  b.naechste_faelligkeit ASC, "
